...
def _to_json(rec: etl.Record):
    ...
    return map(
        lambda relationship: [json.dumps(schema.dump(relationship))], relationships
    )
...
class Service:
    ...
    def get_relationships(self):
        ...
        hierarchy = etl.cut(
            hierarchy, ["IMMEDIATE_PARENT", "CODE", "CODE_TEXT", "LONG_COMMON_NAME"]
        )
        ...
        hierarchy = etl.leftjoin(
            hierarchy,
            parents,
            lkey="IMMEDIATE_PARENT",
            rkey="CODE",
            lprefix="source.",
            rprefix="target.",
        )
